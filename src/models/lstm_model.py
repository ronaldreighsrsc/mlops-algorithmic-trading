    ...
    def find_best_params(self, X_train: np.ndarray, y_train: np.ndarray, param_distributions: dict, n_iter: int = 3, use_bayesian: bool = True) -> dict:
        ...
        if use_bayesian:
            try:
                ...
                print(f"  🔍 Buscando hiperparámetros (Optimización Bayesiana Optuna TPE - Purged CV para LSTM)...")
                ...
                print(f"  ✅ Ganador Bayesiano (Optuna LSTM): {best_params} (Acc Interno: {best_score:.2%})")
                return best_params
